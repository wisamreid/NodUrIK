...
struct UdpSocket {
  ...
  enum { OPTION_UNSPEC=0, OPTION_FORCE_IPV4=1, OPTION_FORCE_IPV6=2, OPTION_ENABLE_BROADCAST=3,
...
private:
  ...
  bool openSocket(const std::string &hostname, const std::string &port, int options) {
    ...
    if (options == OPTION_FORCE_IPV4 || options == OPTION_ENABLE_BROADCAST) hints.ai_family = AF_INET; /* Broadcast is not implemented in IPv6 */
    ...
    for (rp = result; rp && handle==-1; rp = rp->ai_next) {
      ...
      if (options == OPTION_ENABLE_BROADCAST){
        int broadcast = 1;
